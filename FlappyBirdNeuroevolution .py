...
backpng = os.path.join(os.getcwd(), "background.png")
basepng = os.path.join(os.getcwd(), "base.png")
pipepng = os.path.join(os.getcwd(), "pipe.png")
birdpng = os.path.join(os.getcwd(), "bird.png")
...
class GameCore:
    def __init__(self, Population_Number = 1):
        ...
        self.BackGround = pygame.image.load(backpng).convert()

        self.Base = pygame.image.load(basepng).convert()

        self.Pipe_Image = pygame.image.load(pipepng).convert_alpha()
        self.Pipe_Number = 0


        self.Bird_Image = pygame.image.load(birdpng).convert_alpha()
        self.Bird_Mask = pygame.mask.from_surface(self.Bird_Image)

        self.Font_T = pygame.font.SysFont("Times", 40)
    ...
    def GameLoop(self):
        ...
        print(f"Scor = {self.Population[-1].Score} Generation = {self.Generation_Timer} Alive Birds number = {len(self.Population)}")
